...
Disable package since Nix is managing all packages.
#+begin_src elisp
  (setq package-enable-at-startup nil)
#+end_src

Clean up the UI by removing tool bars etc. and add relative line numbers.
#+begin_src elisp
  (tool-bar-mode -1)
  (scroll-bar-mode -1)
  (menu-bar-mode -1)
...
Move files that emacs leaves behind to =~/.emacs.d=
#+begin_src elisp
...
Set indentation to spaces instead of tabs with witdth 2.
#+begin_src elisp
  (setq-default indent-tabs-mode nil)
  (setq-default tab-width 2)
#+end_src

General improvements like pair braces etc.
#+begin_src elisp
...
Make the UI pretty with modus theme, nerd icons and doom modeline.
#+begin_src elisp
...
Completion in editing buffer and when programming.
#+begin_src elisp
(use-package corfu
  :ensure nil
...
Code formatting using apheleia.
#+begin_src elisp
...
Enable which-key mode for key tooltips.
#+begin_src elisp
...
Set up lsp mode for programming.
#+begin_src elisp
...
Use nix mode for programming in nix.
#+begin_src elisp
...
Use rainbow delemiters to see parentheses better in prog mode.
#+begin_src elisp
